...
4. **Lineage graph**: define an asset-to-asset relationship model (edges) and API conventions for querying/upserting it.
5. **Observability**: document SLOs and add a `/metrics` endpoint (Prometheus) when needed.

## Mid-term

* **Ingestion framework**: connector-style ingestion interfaces (HTTP, queued tasks, and job-style execution for long runs).
