...
def test_write_framework_stack_bootstrap_files(tmp_path):
    module = _load_module(SCRIPT_PATH, "configure_knowledge_sources")

    manifest_path = module.write_framework_stack_manifest(tmp_path)
    guide_path = module.write_framework_stack_guide(tmp_path)

    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    assert any(item["name"] == "viewflow" for item in manifest["repositories"])
    behavioral_source = next(
        item
        for item in manifest["behavioral_sources"]
        if item["name"] == "viewflow-demo-site"
    )
    assert behavioral_source["preferred_capture_mode"] == "http"
    assert behavioral_source["browser_dump_commands"]
    assert behavioral_source["playwright_browser_executables"]
    guide = guide_path.read_text(encoding="utf-8")
    assert "demo.viewflow.io" in guide
    assert "capture_behavioral_sources.py" in guide
    assert "--fetch-mode browser" in guide
    assert "--fetch-mode playwright" in guide
    assert " import \\" in guide or "capture_behavioral_sources.py import" in guide
    assert "diff" in guide
...
def test_generate_content_creates_doc_and_skill_entries(tmp_path):
    ...
    (doc_repo / "package.json").write_text(
        json.dumps({"version": "2.2.15"}), encoding="utf-8"
    )
    ...
    assert (
        content_root / "knowledge-src" / "skills" / "cookbook" / "SKILL.md"
    ).exists()
...
def test_generate_content_creates_cookbook_sample_and_theme_skills(tmp_path):
    ...
    (viewflow_dir / "README.md").write_text(
        "# Viewflow\n\nWorkflow docs", encoding="utf-8"
    )
    (viewflow_dir / "package.json").write_text(
        json.dumps({"version": "2.2.15"}), encoding="utf-8"
    )
    (viewflow_dir / "viewflow" / "urls").mkdir(parents=True)
    (viewflow_dir / "viewflow" / "urls" / "sites.py").write_text(
        "class Site: ...\nclass Application: ...", encoding="utf-8"
    )
    (viewflow_dir / "viewflow" / "urls" / "model.py").write_text(
        "class ModelViewset: ...", encoding="utf-8"
    )
    (viewflow_dir / "viewflow" / "workflow" / "flow").mkdir(parents=True)
    (viewflow_dir / "viewflow" / "workflow" / "base.py").write_text(
        "class Node: ...", encoding="utf-8"
    )
    (viewflow_dir / "viewflow" / "workflow" / "activation.py").write_text(
        "class Activation: ...", encoding="utf-8"
    )
    ...
    (viewflow_dir / "viewflow" / "forms" / "renderers.py").write_text(
        "class WidgetRenderer: ...", encoding="utf-8"
    )
    (viewflow_dir / "viewflow" / "utils.py").write_text(
        "def has_object_perm(user, short_perm_name, model, obj=None): ...",
        encoding="utf-8",
    )
    (viewflow_dir / "viewflow" / "templates" / "viewflow").mkdir(parents=True)
    (viewflow_dir / "viewflow" / "templates" / "viewflow" / "base.html").write_text(
        "{% block body %}{% endblock %}", encoding="utf-8"
    )
    (
        viewflow_dir / "viewflow" / "templates" / "viewflow" / "base_page.html"
    ).write_text(
        "{% block page-menu %}{% endblock %}{% block page-toolbar %}{% endblock %}",
        encoding="utf-8",
    )
    (viewflow_dir / "viewflow" / "templates" / "viewflow" / "views").mkdir(parents=True)
    (
        viewflow_dir / "viewflow" / "templates" / "viewflow" / "views" / "list.html"
    ).write_text(
        "{% extends 'viewflow/base_page.html' %}{% block page-toolbar-actions %}<vf-page-search></vf-page-search>{% endblock %}",
        encoding="utf-8",
    )
    (
        viewflow_dir / "viewflow" / "templates" / "viewflow" / "views" / "detail.html"
    ).write_text(
        "{% include 'viewflow/includes/object_detail_card.html' %}", encoding="utf-8"
    )
    (
        viewflow_dir / "viewflow" / "templates" / "viewflow" / "views" / "form.html"
    ).write_text(
        "{% block extrahead %}{{ form.media }}{% endblock %}{% render form form.layout %}",
        encoding="utf-8",
    )
    (
        viewflow_dir
        ...
        / "views"
        / "confirm_delete.html"
    ).write_text("{% trans 'Delete' %}", encoding="utf-8")
    (
        viewflow_dir
        / "viewflow"
        / "templates"
        ...
        "{% extends 'viewflow/views/form.html' %}{% block breadcrumbs_items %}Transition{% endblock %}",
        encoding="utf-8",
    )
    (viewflow_dir / "viewflow" / "templates" / "viewflow" / "includes").mkdir(
        parents=True
    )
    (
        viewflow_dir
        ...
        / "viewflow"
        / "includes"
        / "object_detail_card.html"
    ).write_text(
        "<section class='vf-card__header'></section><section class='mdc-card__actions vf-card__actions'></section>",
        encoding="utf-8",
    )
    (
        viewflow_dir
        ...
        / "viewflow"
        / "includes"
        / "view_action_menu.html"
    ).write_text(
        "{% with actions=view.get_page_actions %}<vf-card-menu></vf-card-menu>{% endwith %}",
        encoding="utf-8",
    )
    (
        viewflow_dir
        ...
        / "includes"
        / "snackbar.html"
    ).write_text("<vf-snackbar>{{ message }}</vf-snackbar>", encoding="utf-8")
    (
        viewflow_dir
        / "viewflow"
        / "templates"
        / "viewflow"
        / "includes"
        / "list_bulk_actions.html"
    ).write_text("Select all<button>GO</button>", encoding="utf-8")
    (
        viewflow_dir
        / "viewflow"
        / "templates"
        ...
        "<vf-page-menu-navigation>{% trans 'Back' %}</vf-page-menu-navigation>",
        encoding="utf-8",
    )
    (
        viewflow_dir
        ...
        "<vf-page-menu-navigation>{% trans 'Applications' %}</vf-page-menu-navigation>",
        encoding="utf-8",
    )
    (
        viewflow_dir
        ...
        "<vf-list-filter><button>Filter</button></vf-list-filter>", encoding="utf-8"
    )
    (
        viewflow_dir
        / "viewflow"
    ...
    ).write_text("1-10 of 10", encoding="utf-8")
    (viewflow_dir / "viewflow" / "views").mkdir(parents=True)
    (viewflow_dir / "viewflow" / "views" / "create.py").write_text(
        "class CreateModelView:\n    def get_template_names(self):\n        return ['app/model_form.html', 'viewflow/views/form.html']\n",
        encoding="utf-8",
    )
    (viewflow_dir / "viewflow" / "views" / "detail.py").write_text(
        "class DetailModelView:\n    template_name = 'viewflow/views/detail.html'\n",
        encoding="utf-8",
    )
    (viewflow_dir / "viewflow" / "views" / "delete.py").write_text(
        "class DeleteModelView:\n    template_name = 'viewflow/views/confirm_delete.html'\n",
        encoding="utf-8",
    )
    (viewflow_dir / "viewflow" / "fsm").mkdir(parents=True)
    (viewflow_dir / "viewflow" / "fsm" / "base.py").write_text(
        "class Transition: ...", encoding="utf-8"
    )
    (viewflow_dir / "viewflow" / "fsm" / "viewset.py").write_text(
        "class FlowViewsMixin: ...", encoding="utf-8"
    )
    (viewflow_dir / "viewflow" / "contrib").mkdir(parents=True)
    (viewflow_dir / "viewflow" / "contrib" / "auth.py").write_text(
        "class AuthenticationForm: ...\nclass AuthViewset: ...", encoding="utf-8"
    )
    (viewflow_dir / "tests" / "workflow").mkdir(parents=True)
    (viewflow_dir / "tests" / "workflow" / "test_flow_viewset__workflow.py").write_text(
        "class TestWorkflowViewestFlow: ...", encoding="utf-8"
    )
    ...
    (viewflow_dir / "tests" / "fsm" / "test_fsm__permissions.py").write_text(
        "class _Publication: ...", encoding="utf-8"
    )

    crud_dir = source_root / "cookbook" / "crud101"
    crud_dir.mkdir(parents=True)
    (crud_dir / "README.md").write_text(
        "# CRUD 101\n\nAdmin CRUD demo", encoding="utf-8"
    )
    (crud_dir / "config").mkdir()
    (crud_dir / "config" / "urls.py").write_text(
        "site = Site(title='Demo')", encoding="utf-8"
    )
    (crud_dir / "atlas").mkdir()
    (crud_dir / "atlas" / "viewset.py").write_text(
        "class AtlasApp(Application): ...", encoding="utf-8"
    )
    (crud_dir / "staff").mkdir()
    (crud_dir / "staff" / "viewset.py").write_text(
        "class EmployeeViewset(ModelViewset): ...", encoding="utf-8"
    )

    workflow_dir = source_root / "cookbook" / "workflow101" / "shipment"
    workflow_dir.mkdir(parents=True)
    (workflow_dir / "flows.py").write_text(
        "class ShipmentFlow(flow.Flow): ...", encoding="utf-8"
    )
    (workflow_dir / "views.py").write_text(
        "class StartView: ...\nclass ShipmentView: ...", encoding="utf-8"
    )

    fsm_dir = source_root / "cookbook" / "fsm101" / "review"
    fsm_dir.mkdir(parents=True)
    (fsm_dir / "viewset.py").write_text(
        "class ReviewViewset(FlowViewsMixin): ...", encoding="utf-8"
    )

    forms_dir = source_root / "cookbook" / "forms101" / "forms"
    forms_dir.mkdir(parents=True)
    (forms_dir / "bank_form.py").write_text(
        "class BankForm:\n    layout = Layout()", encoding="utf-8"
    )
    (forms_dir / "viewset.py").write_text(
        "class Forms(Application): ...\ngeneric.FormView.as_view(template_name='forms/form.html')",
        encoding="utf-8",
    )
    (forms_dir / "views.py").write_text(
    ...
    (forms_dir / "templates" / "forms" / "form.html").write_text(
        "{% render form form.layout %}", encoding="utf-8"
    )
    custom_widget_dir = source_root / "cookbook" / "forms101" / "custom_widget"
    custom_widget_dir.mkdir(parents=True)
    (custom_widget_dir / "forms.py").write_text(
        "class EmailForm:\n    layout = Layout()\nclass OrderForm:\n    total = forms.CharField(widget=forms.TextInput(attrs={'leading-icon': 'attach_money'}))",
        encoding="utf-8",
    )
    (custom_widget_dir / "viewset.py").write_text(
        "class OrderViewset(ModelViewset): ...", encoding="utf-8"
    )

    patterns_dir = source_root / "cookbook" / "patterns" / "config"
    patterns_dir.mkdir(parents=True)
    (patterns_dir / "urls.py").write_text(
        "site = Site(title='Workflow patterns')", encoding="utf-8"
    )
    ...
    (dashboard_dir / "config" / "urls.py").write_text(
        "site = Site(title='Dashboard')", encoding="utf-8"
    )
    (dashboard_dir / "oilngas").mkdir(parents=True)
    (dashboard_dir / "oilngas" / "dashboard.py").write_text(
        "dashboard = Dashboard(app_name='oilngas')", encoding="utf-8"
    )
    ...
    (material_dir / "package.json").write_text(
    ...
    (colors_dir / "colors.css").write_text(
        "--color-primary: #0277bd;", encoding="utf-8"
    )
    (material_dir / "tests").mkdir(parents=True)
    (material_dir / "tests" / "test_views_base.py").write_text(
        "class FormLayoutMixinTests: ...", encoding="utf-8"
    )
    (material_dir / "tests" / "test_views_create.py").write_text(
        "class CreateModelViewTests: ...", encoding="utf-8"
    )
    (material_dir / "demo" / "templates" / "demo").mkdir(parents=True)
    (material_dir / "demo" / "templates" / "demo" / "showcases.html").write_text(
        "{% extends 'material/base_page.html' %}", encoding="utf-8"
    )
    (material_dir / "demo" / "templates" / "demo" / "widgets.html").write_text(
        "{% extends 'material/base_page.html' %}", encoding="utf-8"
    )
    (material_dir / "material" / "templates" / "cotton").mkdir(parents=True)
    (material_dir / "material" / "templates" / "cotton" / "CLAUDE.md").write_text(
        "# Cotton component guide", encoding="utf-8"
    )
    (material_dir / "material" / "templates" / "cotton" / "forms" / "text").mkdir(
        parents=True
    )
    (
        material_dir
        ...
        / "text"
        / "outlined.html"
    ).write_text("<input placeholder=' ' data-input />", encoding="utf-8")
    (material_dir / "material" / "templates" / "cotton" / "forms" / "select").mkdir(
        parents=True
    )
    (
        material_dir
        ...
        / "select"
        / "outlined.html"
    ).write_text("<input up-select-trigger />", encoding="utf-8")
    (material_dir / "material" / "templates" / "cotton" / "forms" / "calendar").mkdir(
        parents=True
    )
    (
        material_dir
        ...
        / "calendar"
        / "date.html"
    ).write_text("<input data-trailing-button />", encoding="utf-8")
    (material_dir / "material" / "templates" / "cotton" / "button").mkdir(parents=True)
    (
        material_dir / "material" / "templates" / "cotton" / "button" / "filled.html"
    ).write_text("<button up-ripple>{{ slot }}</button>", encoding="utf-8")
    (
        material_dir / "material" / "templates" / "cotton" / "button" / "fab.html"
    ).write_text("<button class='rounded-full'>+</button>", encoding="utf-8")
    (material_dir / "material" / "templates" / "cotton" / "card").mkdir(parents=True)
    (
        material_dir / "material" / "templates" / "cotton" / "card" / "elevated.html"
    ).write_text("<div class='bg-surface'>{{ slot }}</div>", encoding="utf-8")
    (material_dir / "material" / "templates" / "cotton" / "table").mkdir(parents=True)
    (
        material_dir / "material" / "templates" / "cotton" / "table" / "container.html"
    ).write_text(
    ...
    assert (
        content_root / "knowledge-src" / "skills" / "cookbook-crud101" / "SKILL.md"
    ).exists()
    assert (
        content_root
        / "knowledge-src"
    ...
    assert (
        "preserving a visible gap between the sidebar and the content column"
        in admin_shell.read_text(encoding="utf-8")
    )
    assert (
        "Choose a Material icon that matches the action context"
        in admin_shell.read_text(encoding="utf-8")
    )
    ...
    assert "Cookbook orchestration example" in workflow_skill.read_text(
        encoding="utf-8"
    )
    ...
    frontend_skill = (
        ...
        / "knowledge-src"
        / "skills"
        / "django-material-frontend-composition"
        / "SKILL.md"
    )
    ...
    assert (
        "Workflow task views using built-in workflow templates"
        in frontend_skill.read_text(encoding="utf-8")
    )
    assert "Input adornment recipe" in frontend_skill.read_text(encoding="utf-8")
    assert (
        "Custom widget forms with leading-icon attrs and computed tags"
        in frontend_skill.read_text(encoding="utf-8")
    )
    ...
    forms_skill = (
        ...
        / "knowledge-src"
        / "skills"
        / "viewflow-forms-layout-composition"
        / "SKILL.md"
    )
    assert forms_skill.exists()
    assert "FieldSet" in forms_skill.read_text(encoding="utf-8")
    assert "FormView registration inside an Application" in forms_skill.read_text(
        encoding="utf-8"
    )
    assert "Leading-icon and custom widget attribute examples" in forms_skill.read_text(
        encoding="utf-8"
    )

    view_recipes_skill = (
        content_root / "knowledge-src" / "skills" / "viewflow-view-recipes" / "SKILL.md"
    )
    ...
    assert (
        "Application-mounted FormView and CreateView routes with titles/buttons"
        in view_recipes_skill.read_text(encoding="utf-8")
    )
    assert (
        "Shared form template with render form form.layout and action footer"
        in view_recipes_skill.read_text(encoding="utf-8")
    )

    crud_templates_skill = (
        content_root
        / "knowledge-src"
        / "skills"
        / "viewflow-crud-page-templates"
        / "SKILL.md"
    )
    assert crud_templates_skill.exists()
    assert "object_detail_card" in crud_templates_skill.read_text(encoding="utf-8")
    assert (
        "Delete and transition confirmation contracts"
        in crud_templates_skill.read_text(encoding="utf-8")
    )
    ...
    includes_skill = (
        ...
        / "knowledge-src"
        / "skills"
        / "viewflow-navigation-feedback-includes"
        / "SKILL.md"
    )
    ...
    base_template_skill = (
        ...
        / "knowledge-src"
        / "skills"
        / "viewflow-base-template-composition"
        / "SKILL.md"
    )
    ...
    custom_renderer_skill = (
        content_root
        / "knowledge-src"
        / "skills"
        / "viewflow-custom-field-renderers"
        / "SKILL.md"
    )
    assert custom_renderer_skill.exists()
    assert "AjaxModelSelectRenderer" in custom_renderer_skill.read_text(
        encoding="utf-8"
    )
    ...
    auth_skill = (
        ...
        / "knowledge-src"
        / "skills"
        / "viewflow-auth-account-scaffolding"
        / "SKILL.md"
    )
    ...
    advanced_flow_skill = (
        ...
        / "knowledge-src"
        / "skills"
        / "viewflow-advanced-flow-topologies"
        / "SKILL.md"
    )
    ...
    assert (
        "Guardian-backed department bill approval flow"
        in assignment_skill.read_text(encoding="utf-8")
    )
    ...
    advanced_ux_skill = (
        ...
        / "knowledge-src"
        / "skills"
        / "django-material-advanced-ux-patterns"
        / "SKILL.md"
    )
    assert advanced_ux_skill.exists()
    assert "Cotton component architecture" in advanced_ux_skill.read_text(
        encoding="utf-8"
    )
    assert "Table container contract" in advanced_ux_skill.read_text(encoding="utf-8")
    assert "Floating action button primitive" in advanced_ux_skill.read_text(
        encoding="utf-8"
    )

    metadata_forms_skill = (
        content_root
        / "knowledge-src"
        / "skills"
    ...
    assert "FormDependentSelectMixin" in metadata_forms_skill.read_text(
        encoding="utf-8"
    )
    ...
    assert "generate_domain_skill_entries" in maintenance_skill.read_text(
        encoding="utf-8"
    )
...
def test_generate_content_includes_cookbook_directory_without_readme(tmp_path):
    ...
    (guardian_dir / "config" / "urls.py").write_text(
        "urlpatterns = []", encoding="utf-8"
    )
    (guardian_dir / "bills").mkdir()
    (guardian_dir / "bills" / "viewsets.py").write_text(
        "class BillsViewset: ...", encoding="utf-8"
    )
